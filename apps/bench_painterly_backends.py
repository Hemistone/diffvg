...
def _build_temp_config() -> tuple[tempfile.TemporaryDirectory[str], Path]:
    temp_dir = tempfile.TemporaryDirectory(prefix="painterly-bench-", dir="/tmp")
    config_path = Path(temp_dir.name) / "no_precondition.toml"
    config_path.write_text("precondition = false\n", encoding="utf-8")
...
def _write_summary_markdown(path: Path, metadata: dict, args: argparse.Namespace, summaries: list[dict]) -> None:
    lines = [
        ...
        "",
        "| backend | paths | ok/repeats | mean_s | median_s | min_s | max_s | last_loss |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in summaries:
        ...
        lines.append(
            ...
            f"{_fmt(row['max_sec'])} | {_fmt(row['last_loss'])} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    for row in summaries:
        if row["successes"] == 0:
            print(
                ...
                f"success=0/{row['repeats']}"
            )
            continue
        print(
            f"[bench] {row['backend']:>13} paths={row['paths']:>4} "
            f"median={row['median_sec']:.4f}s mean={row['mean_sec']:.4f}s "
            f"ok={row['successes']}/{row['repeats']}"
        )
...
def main() -> None:
    ...
    temp_config_ctx: Optional[tempfile.TemporaryDirectory[str]] = None
    config_path: Optional[Path] = None
    if args.config:
        config_path = (REPO_ROOT / args.config).resolve() if not Path(args.config).is_absolute() else Path(args.config)
        if not config_path.is_file():
            raise FileNotFoundError(f"config not found: {config_path}")
    elif not args.precondition:
        temp_config_ctx, config_path = _build_temp_config()
    ...
    try:
        for paths in args.path_counts:
            for backend in args.backends:
                cmd = _build_command(args, target, backend, paths, config_path)
                ...
                for repeat_idx in range(args.repeats):
                    ...
                    print(
                        ...
                        f"{format_duration(result.elapsed_sec)}{loss_text}"
                    )
                    if not result.success:
                        print(textwrap.indent(result.stdout_tail or result.stderr_tail, prefix="        "))
    finally:
